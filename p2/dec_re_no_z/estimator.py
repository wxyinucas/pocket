...
class Estimator(Data):
    ...
    def __init__(self, parameters, base, sigma, n_samples=200):
        super(Estimator, self).__init__(parameters, n_samples)

        self.lambda_y_a_hat = None

        a_init = np.array([np.random.normal(self.true_alpha1, sigma), np.random.normal(self.true_alpha2, sigma)])
        b_init = np.array(
            [np.random.normal(self.true_beta1, 1.5 * sigma), np.random.normal(self.true_beta2, 1.5 * sigma)])
    ...
    def equation_sn(self, a):
        ...
        t_star_a = make_a_star(self.t, np.exp(self.x * -1))  # 用于测试
        y_star_a = make_a_star(self.y, np.exp(self.x * -1))  # 用于测试
        exp = np.exp(self.x * (a2 - a1))  # 用于测试
