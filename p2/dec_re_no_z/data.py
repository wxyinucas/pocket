...
class Data:
    ...
    def __init__(self, parameters: np.array, n_sample=200):
        ...
        self.df = pd.DataFrame(data=values, columns=names)
    ...
    def cov(self):
        ...
        return np.random.uniform(0.5, 1, self.n_sam)

    # 潜变量
    def latent(self):
        """生成服从Gamma（2， 5）的潜变量。"""
        # return np.random.gamma(2, 5, self.n_sam)
        return np.ones(self.n_sam)
    ...
    def recurrent(self):
        """生成复发时间发生总数和每次的时间。"""
        r_lamb = 1 / 10 * self.z * np.exp(self.x * (self.true_alpha1 + self.true_alpha2)) * (self.y ** 2)
