...
"""
==================================
# Time      : 2018/9/27  09:47
# File      : var_data_test.py
...
class TestData:
    ...
    def gen(self, n_sample, pr):
        ...
        t = np.array([])
        self.m = np.array([])
        for ite in range(n_sample):
            time_list = [0]
            while time_list[-1] < self.c[ite]:
                time_list.append(time_list[-1] + np.random.exponential(scale=1/lamb[ite]))
            else:
                time_list.pop(0)
                time_list.pop()
            self.m = np.append(self.m, len(time_list))
            t = np.array([*t, np.array(time_list)])
